...
import com.lxl.generator.consts.ServiceConst;
...
public class ServiceService extends BaseService {
    ...
    protected void generator() {
        ...
        for (TableExtend tableExtend : list) {
            if (Arrays.asList(ServiceConst.TIME_FIELD).contains(tableExtend.getColumnName()
                    .replaceAll(tableName + ServiceConst.CHARACTER_UNDERLINE, ModelConst.CHARACTER_NULL))) {
    ...
    protected String getPath() {
        if (this.path == null) {
            this.path = System.getProperty("user.dir") + ServiceConst.DIR_SPLIT + ServiceConst.PATH_INNER_JAVA
                    + ServiceConst.DIR_SPLIT + formatPackageDir(packageName) + ServiceConst.PATH_INNER_COMMON_SERVICE;
        }
        return this.path;
    }
